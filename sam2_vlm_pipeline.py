...
try:
    ...
    raise ImportError(
        "SAM2 library is required. Install with `pip install git+https://github.com/facebookresearch/sam2.git`"
    )
...
def analyze_crop(analyzer: SceneAnalyzer, crop: Image.Image) -> Dict[str, str]:
    objects = analyzer.identify_objects(crop)
    name = objects[0]["name"] if objects else "unknown"
    acts = analyzer.identify_activities(crop)
    action = acts[0] if acts else ""
    scene = analyzer.get_scene_description(crop)
    return {"name": name, "action": action, "scene": scene}
...
def process_video(
    ...
    while True:
        ...
        for m, obj_id in zip(masks, ids):
            ...
            info = analyze_crop(analyzer, pil_img)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SAM2 + SmolVLM analysis pipeline")
